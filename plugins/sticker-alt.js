...
module.exports = {  
    command: 'ملصق',  
    aliases: ['sticker', 'ستيكر', 's'],  
    category: 'اوامـࢪ الاداوات',  
    description: 'تحويل صورة أو فيديو إلى ملصق',  
    usage: '.ملصق (رد على صورة أو فيديو)',  
        ...
        try {  
            ...
            if (!quotedMsg) {  
                await sock.sendMessage(chatId, { 
                    text: '⚠️ لازم ترد على صورة أو فيديو عشان أحوله لملصق.' 
                }, { quoted: message });  
                return;  
            }  
            ...
            if (!['imageMessage', 'videoMessage'].includes(type)) {  
                await sock.sendMessage(chatId, { 
                    text: '⚠️ الرد لازم يكون على صورة أو فيديو.' 
                }, { quoted: message });  
                return;  
            }  
            ...
            const tempInput = path.join(
                tempDir, 
                `temp_${Date.now()}.${type === 'imageMessage' ? 'jpg' : 'mp4'}`
            );  
            ...
            await sock.sendMessage(chatId, { 
                text: '❌ فشل إنشاء الملصق، حاول مرة أخرى.' 
            }, { quoted: message });  
        }  
    }  
};